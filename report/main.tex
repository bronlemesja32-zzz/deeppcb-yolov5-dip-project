% !TeX program = xelatex
% 基于 YOLOv5s 的 PCB 表面缺陷检测与图像处理辅助分析 —— 数字图像处理课程大作业报告(初稿)
% 编译: xelatex -> bibtex -> xelatex x2
...
\twocolumn[{
\begin{center}
  {\LARGE\bfseries 基于 YOLOv5s 的 PCB 表面缺陷检测与图像处理辅助分析}\\[10pt]
  {数字图像处理课程大作业报告}\\[4pt]
  {中山大学数学学院（珠海）\quad 覃天祥 \quad 23339096}\\[4pt]
  {\small 2026 年 7 月}
\end{center}
\vspace{4pt}
\begin{quote}\small
\textbf{摘要：}本文基于公开 DeepPCB 数据集完成 PCB 表面缺陷检测课程项目。项目首先将
...
表现较好，而传统差分方法能够提供直观的候选区域和错误分析依据。\\[2pt]
\textbf{关键词：}PCB 缺陷检测；YOLOv5s；DeepPCB；模板差分；Otsu 阈值；形态学处理；连通域分析
\end{quote}
\vspace{10pt}
}]
...
\input{tables/tab_split}
\input{tables/tab_classmap}
...
\input{tables/tab_yolo_overall}
\input{tables/tab_yolo_perclass}
...
\input{tables/tab_ip_metrics}
\input{tables/tab_ip_params}
\input{tables/tab_pad_sweep}
